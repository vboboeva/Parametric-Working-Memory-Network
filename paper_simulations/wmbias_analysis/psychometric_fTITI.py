...
for i, ITI in enumerate(ITIvals):
	...
	axs.errorbar(dstim_set, psy, yerr=sd/np.sqrt(numpts), color=colors[i], label='%s s'%(ITI+1.))
...
fig.legend()
fig.savefig("figs/psycho_%s_ISI%s.png"%(SimulationName,ISI), bbox_inches='tight')
fig.savefig("figs/psycho_%s_ISI%s.svg"%(SimulationName,ISI), bbox_inches='tight')
